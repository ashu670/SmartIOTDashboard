...
// Feature bullet groups for second-phase rotation (Register page)
const REGISTER_BULLET_GROUPS = [
  [
    'Register your house once and control all devices centrally.',
    'Add family members with secure, role-based access.',
    'Approve devices, track activity and stay in control.',
  ],
  [
    'Create your smart home space in just a few simple steps.',
    'Connect and organize devices room-by-room effortlessly.',
    'Get started instantly with a clean and guided setup flow.',
  ],
  [
    'Assign admin and member roles with clear permissions.',
    'Protect your home with secure login and access control.',
    'Ensure only trusted members can manage critical devices.',
  ],
  [
    'Expand your smart home anytime by adding new devices.',
    'Monitor activity logs to keep your home transparent and safe.',
    'Manage everything from one dashboard as your family grows.',
  ],
];
...
export default function RegisterPage() {
  ...
  const [role] = useState('admin'); // admin-only registration
  ...
  // Initial staggered text animation (same timing as Login page)
  useEffect(() => {
    const timeouts = [];
  ...
  // Auto-rotate bullet groups
  useEffect(() => {
    if (!rotationStarted) return;

    const interval = setInterval(() => {
      setCurrentGroupIndex((prev) => (prev + 1) % REGISTER_BULLET_GROUPS.length);
    }, 4000);
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-5xl bg-slate-950/50 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl shadow-slate-900/60 flex flex-col lg:flex-row">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            >
              Create your <span className="font-bold">family home</span> space
            </h1>
            <p
            ...
            >
              As the Family Head (Admin), you&apos;ll manage devices, invite
              members and keep your house running smoothly from one place.
            </p>

            {/* Bullet groups + indicators, matching Login page behavior */}
            <div className="mt-8">
              <div
              ...
              >
                {(() => {
                  const bullets = REGISTER_BULLET_GROUPS[currentGroupIndex];
                  return (
                    <>
                      <div
              ...
              <div
                className={`mt-4 flex justify-center gap-2 transition-all duration-700 ease-out ${
                  showIndicators ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'
                }`}
              >
                {REGISTER_BULLET_GROUPS.map((_, idx) => {
          ...
          <div className="relative z-10 mt-10 hidden md:flex justify-between items-center text-xs text-blue-50/80">
            <p>One admin per house for clear, simple control.</p>
            <p className="text-blue-100 font-medium">Set up • Invite • Manage</p>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 bg-slate-950 px-6 py-8 sm:px-10 sm:py-10 flex items-center justify-center">
          <div className="w-full max-w-md">
            ...
            <p className="text-sm text-slate-400 mb-8">
              This account will be the Family Head for your house. You can add
              family members later from the Admin Panel.
            </p>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Choose a unique house identifier"
                  className="block w-full rounded-xl border border-slate-700 bg-slate-900/70 px-3 py-2.5 text-sm text-slate-100 placeholder-slate-500 shadow-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40 transition"
                  required
                />
                <p className="mt-1 text-xs text-slate-400">
                  Only one admin per house name. Family members will join this
                  house.
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  value="Admin (Family Head)"
                  readOnly
                  className="block w-full rounded-xl border border-slate-700 bg-slate-900/50 px-3 py-2.5 text-sm text-slate-300 shadow-sm"
                />
                <p className="mt-1 text-xs text-slate-400">
                  Only the family head can register here. You&apos;ll invite
                  other members later.
                </p>
              </div>
              ...
              <button
                ...
                className="mt-2 inline-flex w-full items-center justify-center rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-medium text-white shadow-lg shadow-emerald-500/40 transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-70"
              >
